...
pub async fn list_voice_history(
    ...
    if conversation_id.trim().is_empty() {
        return Err(map_error(IpcError::bad_request("conversationId is required")));
    }
    let pool = storage(&state)?;
    let repo = OmniHistoryRepository::new(pool);
    let items = repo.list(&conversation_id, limit, before.as_deref())
        .await
...
#[tauri::command]
pub async fn clear_voice_history(state: State<'_, IpcState>) -> Result<OmniHistoryListResponse, String> {
    let pool = storage(&state)?;
    let repo = OmniHistoryRepository::new(pool);
    repo.clear().await
        .map_err(|e| map_error(IpcError::internal(format!("database error: {}", e))))?;
...
mod tests {
    ...
    fn test_voice_history_response_serialization() {
        let response = OmniHistoryListResponse {
            data: vec![],
        };
