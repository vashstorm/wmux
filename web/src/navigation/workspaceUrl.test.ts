...
describe("formatWorkspaceUrl", () => {
  test("formats valid locations and encodes special characters", () => {
    expect(formatWorkspaceUrl({ connection: "local", session: "wmux" })).toBe(
      "?connection=local&session=wmux",
    )
    expect(formatWorkspaceUrl({ connection: "local", session: "wmux", window: "@1" })).toBe(
      "?connection=local&session=wmux&window=%401",
    )
    expect(
      formatWorkspaceUrl({ connection: "local", session: "wmux", window: "@1", pane: "%1" }),
    ).toBe("?connection=local&session=wmux&window=%401&pane=%251")
    expect(formatWorkspaceUrl({ connection: "local", session: "my session#name" })).toBe(
      "?connection=local&session=my+session%23name",
    )
  })
...
describe("toSelectedPane", () => {
  test("maps WorkspaceLocation fields correctly", () => {
    ...
    expect(
      toSelectedPane({ connection: "conn1", session: "sess1", window: "@5", pane: "%10" }),
    ).toEqual({
      targetName: "conn1",
      session: "sess1",
      window: "@5",
...
describe("fromSelectedPane", () => {
  test("maps SelectedPane fields correctly", () => {
    ...
    expect(
      fromSelectedPane({ targetName: "conn1", session: "sess1", window: "@5", pane: "%10" }),
    ).toEqual({
      connection: "conn1",
      session: "sess1",
      window: "@5",
...
describe("isStructurallyValidWorkspaceLocation", () => {
  test("accepts valid locations", () => {
    expect(isStructurallyValidWorkspaceLocation({ connection: "local", session: "wmux" })).toBe(
      true,
    )
    expect(
      isStructurallyValidWorkspaceLocation({ connection: "local", session: "wmux", window: "@1" }),
    ).toBe(true)
    expect(
      isStructurallyValidWorkspaceLocation({
        connection: "local",
  ...
  test("rejects incomplete or malformed locations", () => {
    expect(isStructurallyValidWorkspaceLocation({ session: "wmux" } as WorkspaceLocation)).toBe(
      false,
    )
    expect(isStructurallyValidWorkspaceLocation({ connection: "local" } as WorkspaceLocation)).toBe(
      false,
    )
