...
function mockAudioContext() {
	const ctx = {
		...
		createScriptProcessor: vi.fn().mockReturnValue({
			connect: vi.fn(),
			disconnect: vi.fn(),
			onaudioprocess: null as ((e: { inputBuffer: { getChannelData: (n: number) => Float32Array; sampleRate: number } }) => void) | null,
		}),
...
describe("AudioPipeline", () => {
	...
	test("VAD skips low-level frames", async () => {
		type AudioProcessEvent = { inputBuffer: { getChannelData: (n: number) => Float32Array; sampleRate: number } };
		const scriptNode: {
	...
	test("resamples PCM when sample rates differ", async () => {
		...
		const result = (pipeline as unknown as { resamplePCM: (i: Float32Array, f: number, t: number) => Float32Array }).resamplePCM(input, 8000, 16000);
