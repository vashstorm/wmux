...
export function parseAiUsageResponse(responseJson: string | null | undefined): ParsedAiUsageResponse {
	if (!responseJson) {
		return { formatted: null, content: null, contentJson: null, parseError: false, contentParseError: false };
	}

	const parsed = parseJsonRecord(responseJson);
	if (!parsed) {
		return { formatted: responseJson, content: null, contentJson: null, parseError: true, contentParseError: false };
	}

	const choices = Array.isArray(parsed.choices) ? parsed.choices : [];
	const firstChoice = choices[0];
	const firstMessage = isRecord(firstChoice) && isRecord(firstChoice.message) ? firstChoice.message : null;
	...
	const contentParseError = typeof content === "string" && content.trim().length > 0 && contentRecord === null;
...
export function isProjectAiHtmlEvent(event: AiUsageEvent, parsed = parseAiUsageResponse(event.responseJson)): boolean {
	return event.targetName === "project" || parsed.operation === "generate_ai_html";
}

export function getAiUsageKindLabel(event: AiUsageEvent, parsed = parseAiUsageResponse(event.responseJson)): string {
	if (isProjectAiHtmlEvent(event, parsed)) return "Project HTML";
	return "Window Analysis";
}

export function getAiUsageTitle(event: AiUsageEvent, parsed = parseAiUsageResponse(event.responseJson)): string {
	if (isProjectAiHtmlEvent(event, parsed)) return parsed.projectName ?? event.sessionName ?? "Project HTML";
	return event.sessionName || parsed.app || event.model || "Window Analysis";
}

export function getAiUsageSubtitle(event: AiUsageEvent, parsed = parseAiUsageResponse(event.responseJson)): string {
