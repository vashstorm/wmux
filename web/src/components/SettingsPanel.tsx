...
function groupOmniSkills(skills: OmniSkillDef[]): SkillGroup[] {
  const grouped = new Map<string, SkillGroup>()

  for (const skill of skills) {
    const sourceFile = skill.sourceFile ?? `${skill.id}.md`
    const existing = grouped.get(sourceFile) ?? { key: sourceFile, label: sourceFile, skills: [] }
  ...
  return Array.from(grouped.values())
    ...
    .map((group) => ({
      ...
      skills: group.skills.sort((a, b) => {
        const orderA = a.sourceOrder ?? Number.MAX_SAFE_INTEGER
        const orderB = b.sourceOrder ?? Number.MAX_SAFE_INTEGER
...
function buildFormState(config: AppConfig): SettingsFormState {
  ...
  return {
    ...
    editingSkill: null,
    omniSkValue:
      config.omni?.dashscopeApiKey ??
      (config.omni?.dashscopeApiKeyConfigured
        ? "••••••••••••••••••••••••••••••••••••••••••••••••••"
        : ""),
  }
}
...
export function SettingsPanel() {
  ...
  const skillGroups = useMemo(
    () => groupOmniSkills(formState?.omniSkills ?? []),
    [formState?.omniSkills],
  )
  const enabledSkillCount = useMemo(
    () => formState?.omniSkills.filter((skill) => skill.enabled).length ?? 0,
    [formState?.omniSkills],
  )
    ...
    return {
      ...
      omni: {
        ...
        dashscopeApiKeyConfigured:
          formState.omniSkConfigured ||
          (formState.omniSkValue.trim().length > 0 &&
            formState.omniSkValue !== "••••••••••••••••••••••••••••••••••••••••••••••••••"),
        dashscopeApiKey:
          formState.omniSkValue.trim() &&
          formState.omniSkValue !== "••••••••••••••••••••••••••••••••••••••••••••••••••"
            ? formState.omniSkValue.trim()
            : undefined,
  ...
  const performSave = async (payload: AppConfig) => {
    ...
    try {
      ...
      const savedScaleStep =
        saved.ui.uiScaleStep !== undefined
          ...
          onRetry: async () => {
            ...
            const retryPayload: AppConfig = {
              ...
              ui: {
                ...
                windowTheme: payload.ui.windowTheme,
                uiScaleStep: payload.ui.uiScaleStep,
                fontSize: payload.ui.fontSize,
                terminalFontSize: payload.ui.terminalFontSize,
                terminalFontWeight: payload.ui.terminalFontWeight,
              },
              intelligence: {
                ...latest.intelligence,
              ...
              connections: latest.connections.map((connection) => {
                ...
                const pendingConnection = payload.connections.find(
                  (item) => item.targetName === connection.targetName,
                )
                return {
                  ...connection,
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    if (formState.intelligenceEnabled) {
      if (formState.intelligenceProviders.length === 0) {
        setError({
          code: "bad_request",
          message: "At least one provider is required when intelligence is enabled",
        })
        return
      }
      if (!formState.intelligenceActiveProvider.trim()) {
        setError({
          code: "bad_request",
          message: "An active provider must be selected when intelligence is enabled",
        })
        return
      }
      ...
      for (const provider of formState.intelligenceProviders) {
        ...
        if (names.has(provider.name)) {
          setError({
            code: "bad_request",
            message: `Provider name "${provider.name}" must be unique`,
          })
          return
        }
        ...
        ? {
            ...
            omni: current.omni
              ? {
                  ...current.omni,
                  skillDefinitions: [
                    ...(current.omni.skillDefinitions ?? []).filter((item) => item.id !== skill.id),
                    skill,
  ...
  const handleEditSkill = (skill: OmniSkillDef) => {
    setFormState((current) =>
      current ? { ...current, editingSkill: skillToEditor(skill) } : current,
    )
  }
            ...
            current
              ? {
                  ...
                  omni: current.omni
                    ? {
                        ...current.omni,
                        skillDefinitions: (current.omni.skillDefinitions ?? []).filter(
  ...
  const renderProviderEditor = (editor: ProviderFormState) => {
    return (
      <Box
        className="intelligence-provider-editor"
        data-testid="intelligence-provider-editor"
        sx={{
          display: "flex",
        ...
        <Box sx={{ display: "flex", gap: 1, justifyContent: "flex-end" }}>
          <Button
            ...
            onClick={() => {
              ...
              setFormState((current) => (current ? { ...current, editingProvider: null } : current))
            }}
          >
            CANCEL
          </Button>
          <Button
            ...
            onClick={() => {
              ...
              if (
                ...
                editor.apiKeyInput !== MASKED_API_KEY
              ) {
              ...
              const duplicateName = formState.intelligenceProviders.some(
                (p) =>
                  p.name === editor.name.trim() &&
                  p.name !== (editor.isNew ? "" : editor.originalName),
              )
              const existingProvider =
                formState.editingProvider && !formState.editingProvider.isNew
                  ? formState.intelligenceProviders.find((p) => p.name === editor.originalName)
                  : undefined
              if (duplicateName && existingProvider?.name !== editor.name.trim()) {
                setError({
                  code: "bad_request",
                  message: `Provider name "${editor.name.trim()}" already exists`,
                })
                return
              }
              ...
              setFormState((current) => {
                ...
                const existingIndex = current.intelligenceProviders.findIndex(
                  (p) =>
                    p.name ===
                      (isNew
                        ? current.editingProvider?.name
                        : current.editingProvider?.originalName) && !isNew,
                )
                ...
                return {
                  ...
                  intelligenceActiveProvider:
                    current.intelligenceActiveProvider === current.editingProvider.originalName
  ...
  const renderSkillEditorDialog = () => {
    ...
    return (
      <Dialog
        open={Boolean(editor)}
        onClose={() =>
          setFormState((current) => (current ? { ...current, editingSkill: null } : current))
        }
        maxWidth="md"
        fullWidth
            ...
            <>
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1, pt: 1 }}>
                <Button
                  ...
                  onClick={() =>
                    setFormState((current) =>
                      current ? { ...current, editingSkill: null } : current,
                    )
                  }
                >
                  Cancel
      ...
      >
        ...
        >
          <Box className="settings-panel-header-title">
            <Typography variant="h6" component="h3" className="form-title">
              Settings
            </Typography>
          </Box>
          <IconButton
          ...
          >
            ...
            >
                        ...
                        >
                          {item.key === "general"
                            ...
                            : item.key === "connections"
                              ...
                              : item.key === "typography"
                                ? `scale ${formState.uiScaleStep}`
                                : item.key === "intelligence"
                                  ? formState.intelligenceEnabled
                                    ? "Enabled"
                                    : "Disabled"
                                  : item.key === "omni-skills"
                                    ? `${skillGroups.length} groups`
                                    : formState.omniEnabled
                                      ? "Enabled"
                                      : "Disabled"}
                        </Typography>
                      </Box>
              ...
              <Box className="settings-sidebar-footer" sx={{ mt: "auto", p: 1 }}>
                <Typography variant="caption" color="text.secondary" className="version-info">
                  wmux v{window.performance ? "0.1.0" : "dev"}
                </Typography>
              </Box>
            </Box>
                ...
                >
                    ...
                    >
                      ...
                      <Box>
                        ...
                        <TextField
                          ...
                          placeholder={
                            formState.tokenConfigured ? "••••••••••••••••" : "Optional on localhost"
                          }
                        ...
                        >
                          {formState.tokenConfigured
                            ? "A token is configured. Enter a new value to replace it."
                      ...
                      {connections.length === 0 ? (
                        ...
                        <Box component="ul" sx={{ listStyle: "none", p: 0, m: 0 }}>
                          {connections.map((connection) => {
                            ...
                            const subtitle =
                              connection.type === "ssh" && connection.host
                            ...
                            return (
                              ...
                              >
                                <Box
                                  className="settings-connection-info"
                                ...
                                >
                                  ...
                                  <Box className="settings-connection-details">
                                    ...
                                    >
                                      <Typography
                                        className="settings-connection-name"
                                        variant="body2"
                                        sx={{ fontWeight: 700 }}
                                      >
                                    ...
                                    <Typography
                                      className="settings-connection-meta"
                                      variant="caption"
                                      color="text.secondary"
                                    >
                                      {subtitle}
                                ...
                                <Box
                                  className="settings-connection-actions"
                    ...
                    >
                      <Box>
                        ...
                        <FormControlLabel
                          control={
                            <Switch
                              ...
                              onChange={(event) =>
                                updateField("intelligenceEnabled", event.target.checked)
                              }
                              slotProps={{
                                input: {
                        ...
                        <Typography
                          ...
                          data-testid="intelligence-projects-context"
                        >
                          Projects use the active AI provider to generate window analysis HTML
                          summaries for your project dashboard.
                        </Typography>
                      </Box>
                      ...
                      <Box>
                        ...
                        {formState.editingProvider?.isNew &&
                          renderProviderEditor(formState.editingProvider)}

                        {formState.intelligenceProviders.length === 0 &&
                        !formState.editingProvider ? (
                          <Box
                            className="intelligence-providers-empty"
                            data-testid="intelligence-providers-empty"
                          ...
                          <Box
                            component="ul"
                            className="intelligence-providers-list"
                            data-testid="intelligence-providers-list"
                            sx={{
                              listStyle: "none",
                              p: 0,
                              m: 0,
                          ...
                          >
                            {formState.intelligenceProviders.map((provider) => {
                              const isActive =
                                provider.name === formState.intelligenceActiveProvider
                              const isEditing =
                                formState.editingProvider?.originalName === provider.name &&
                                !formState.editingProvider.isNew
                              return (
                                <Box
                                  key={provider.name}
                                ...
                                >
                                  ...
                                  >
                                    <Box
                                      className="intelligence-provider-info"
                                      sx={{
                                        display: "flex",
                                        flexDirection: "column",
                                        gap: 0.5,
                                    ...
                                    >
                                      ...
                                      >
                                        <Typography
                                          className="intelligence-provider-name"
                                          variant="body2"
                                          sx={{ fontWeight: 700 }}
                                          noWrap
                                        >
                                      ...
                                      <Box
                                        className="intelligence-provider-meta"
                                      ...
                                      >
                                        ...
                                        />
                                        <Typography
                                          className="intelligence-provider-model"
                                          component="span"
                                          variant="caption"
                                          color="text.secondary"
                                        >
                                    ...
                                    <Box
                                      className="intelligence-provider-actions"
                                      sx={{ display: "flex", gap: 0.5, flexShrink: 0 }}
                                    >
                                      <Tooltip
                                        title={
                                          isActive ? "Active provider" : "Set as active provider"
                                        }
                                      >
                                        <span>
                                          <IconButton
                                            ...
                                            onClick={() =>
                                              setFormState((current) =>
                                                current
                                                  ? {
                                                      ...current,
                                                      intelligenceActiveProvider: provider.name,
                                                    }
                                                  : current,
                                              )
                                            }
                                            ...
                                            aria-label={
                                              isActive
                                                ? "Active provider"
                                                : `Set ${provider.name} as active provider`
                                            }
                                          ...
                                          <IconButton
                                            ...
                                            onClick={() => {
                                              setFormState((current) => {
                                                if (!current) return current
                                                const providers =
                                                  current.intelligenceProviders.filter(
                                  ...
                                  {isActive && (
                                    <div
                                      className="intelligence-provider-active-indicator"
                                      data-testid={`provider-active-indicator-${provider.name}`}
                                    />
                                  )}
                                </Box>
                              )
                      ...
                      <Box>
                        ...
                        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                          <TextField
                            ...
                            onChange={(event) =>
                              updateField("intelligenceMaxBytes", Number(event.target.value))
                            }
                            disabled={!formState.intelligenceEnabled}
                            fullWidth
                            slotProps={{
                          ...
                          <TextField
                            ...
                            onChange={(event) =>
                              updateField("intelligenceTimeoutSec", Number(event.target.value))
                            }
                            disabled={!formState.intelligenceEnabled}
                            fullWidth
                            slotProps={{
                          ...
                          <TextField
                            ...
                            onChange={(event) =>
                              updateField(
                                "intelligenceMinSessionIntervalSec",
                          ...
                          <TextField
                            ...
                            onChange={(event) =>
                              updateField("intelligenceMaxConcurrency", Number(event.target.value))
                            }
                            disabled={!formState.intelligenceEnabled}
                            fullWidth
                            slotProps={{
                          ...
                          <TextField
                            ...
                            onChange={(event) =>
                              updateField("intelligenceCacheTTLSec", Number(event.target.value))
                            }
                            disabled={!formState.intelligenceEnabled}
                            fullWidth
                            slotProps={{
                  ...
                  {activeTab === "omni" && (
                    ...
                    >
                      ...
                      <VoiceSettingsTab omniVoice={formState.omniVoice} updateField={updateField} />
                    </Box>
                  )}
                      ...
                      >
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {skillGroups.length} groups, {formState.omniSkills.length} tools,{" "}
                      ...
                      {formState.omniSkills.length === 0 ? (
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                          {skillGroups.map((group) => {
                            const enabledInGroup = group.skills.filter(
                              (skill) => skill.enabled,
                            ).length
                            return (
                              <Accordion
                                key={group.key}
                                expanded={expandedSkillGroups[group.key] ?? false}
                                onChange={() =>
                              ...
                              >
                                <AccordionSummary
                                  ...
                                  aria-controls={`skill-group-${group.key}-content`}
                                  id={`skill-group-${group.key}-header`}
                                  data-testid={`omni-skill-group-${group.key}`}
                                  sx={{
                                    minHeight: 48,
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <Chip
                                      label={`${group.skills.length} tools`}
                                      size="small"
                                      variant="outlined"
                                    />
                                  </Stack>
                                  <Typography
                                    ...
                                    sx={{ flexShrink: 0 }}
                                  >
                                    {enabledInGroup}/{group.skills.length} enabled
                                  </Typography>
                                </AccordionSummary>
                                <AccordionDetails
                                  id={`skill-group-${group.key}-content`}
                                  sx={{ pt: 0, pb: 1.25, px: 1.25 }}
                                >
                                  <Box
                                    component="ul"
                                    sx={{
                                      ...
                                      >
                                        ...
                                        >
                                          <Switch
                                            ...
                                            onChange={(event) => {
                                              void handleToggleSkill(skill, event.target.checked)
                                            }}
                                            slotProps={{
                                              input: {
...
ui: scale ${formState.uiScaleStep > 0 ? "+" : ""}${formState.uiScaleStep}`}</Typography>
                          ...
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            <IconButton
                              ...
                              onClick={() =>
                                updateField(
                                  "uiScaleStep",
                                  clampUIScaleStep(formState.uiScaleStep - 1),
                                )
                              }
                            ...
                            >
                              {formState.uiScaleStep > 0
                                ? `+${formState.uiScaleStep}`
                                : formState.uiScaleStep}
                            </Typography>
                            <IconButton
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
                          <FormControl fullWidth>
                            <InputLabel htmlFor="settings-terminal-font-weight">
                            ...
                            <Select
                              ...
                              onChange={(event) =>
                                updateField(
                                  "terminalFontWeight",
                                  normalizeTerminalFontWeight(event.target.value),
                                )
                              }
                              inputProps={{
                                id: "settings-terminal-font-weight",
                            ...
                            <Typography variant="caption" color="text.secondary" sx={{ mt: 0.75 }}>
                              Font weight for terminal text. Size follows UI scale step.
                            </Typography>
                          </FormControl>
                        </Box>
                      </Box>
                ...
                >
                  <Box
                    ...
                    sx={{ minHeight: 24, display: "flex", alignItems: "center", gap: 1 }}
                  >
                    {isSaving && (
                      <>
