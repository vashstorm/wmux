...
vi.mock("./PaneCanvas.js", () => ({
  PaneCanvas: vi.fn(
    ({ panes, selectedPaneId }: { panes: PaneData[]; selectedPaneId: string | null }) => (
...
describe("MainPanel", () => {
  ...
  test("keeps the selected window when tmux active window changes externally", async () => {
    vi.mocked(useAppState).mockReturnValue({
      ...
      sessions: {
        "conn-1": [
          { name: "dev", intelligenceApp: "claude", intelligenceSummary: "Waiting for input" },
        ],
      },
      windows: {
        "conn-1:dev": {
    ...
    expect(setWindows).toHaveBeenCalledWith(
      "conn-1",
      "dev",
      expect.arrayContaining([activeWindowTwo]),
    )
    expect(listPanes).toHaveBeenCalledWith("conn-1", "dev", "@1")
    expect(setPanes).toHaveBeenCalledWith("conn-1", "dev", "@1", [
  ...
  test("keeps the selected pane when tmux active pane changes externally", async () => {
    ...
    expect(setPanes).toHaveBeenCalledWith(
      ...
      "dev",
      "@1",
      expect.arrayContaining([expect.objectContaining({ ID: "%2", Active: true })]),
    )
    expect(setSelectedPane).not.toHaveBeenCalled()
  })
  ...
  test("renders title as summary without session or app name", () => {
    vi.mocked(useAppState).mockReturnValue({
      ...
      sessions: {
        "conn-1": [
          { name: "dev", intelligenceApp: "claude", intelligenceSummary: "Session fallback" },
        ],
      },
      windows: {
        "conn-1:dev": {
  ...
  test("falls back to window name and pane title when current window has no intelligence metadata", () => {
    vi.mocked(useAppState).mockReturnValue({
      ...
      sessions: {
        "conn-1": [
          { name: "wmux", intelligenceApp: "opencode", intelligenceSummary: "OpenCode CLI 已启动" },
        ],
      },
      windows: {
        "conn-1:wmux": {
