...
export function MainPanel() {
  ...
  const sessionKey = selectedPane ? `${selectedPane.targetName}:${selectedPane.session}` : null
  ...
  const selectedSession = selectedPane
    ? (sessions[selectedPane.targetName] ?? []).find(
        (session) => session.name === selectedPane.session,
      )
    : null
  ...
  const handleSelectWindow = useCallback(
    async (windowId: string, activePaneId: string, options: SelectWindowOptions = {}) => {
      if (!selectedPane) return

      const loadedPanesForWindow = sessionWindowState?.loadedPanes[windowId]
      const optimisticPaneId =
        loadedPanesForWindow?.find((pane) => pane.active)?.id ??
      ...
      if (options.forcePanes || !loadedPanesForWindow || loadedPanesForWindow.length === 0) {
        try {
          ...
          if (err instanceof Error && "code" in err) {
            ...
            setError({
              code: "unknown_error",
              message: err instanceof Error ? err.message : "Failed to load panes",
            })
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const syncActiveWindow = async () => {
      ...
      inFlight = true
      try {
        const windowsResponse = await listWindows(selectedPane.targetName, selectedPane.session)
        if (cancelled) return
  ...
  return (
    <div className="main-panel">
      ...
      <main
        className={`main-content${hasSelectedPane || selectedProject || selectedAiEvent || selectedAiLog ? " has-workspace" : " is-empty"}`}
      >
        {selectedAiEvent ? (
