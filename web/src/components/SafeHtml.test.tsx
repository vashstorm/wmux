...
describe("SafeHtml", () => {
	...
	test("strips script tags", () => {
		const { container } = render(
			<SafeHtml html='<div>safe</div><script>alert("xss")</script>' />,
		);
	...
	test("strips iframe tags", () => {
		const { container } = render(<SafeHtml html='<iframe src="https://evil.com"></iframe><p>safe</p>' />);
		expect(container.querySelector("iframe")).toBeNull();
		expect(screen.getByText("safe")).toBeInTheDocument();
	});

	test("strips form tags", () => {
		const { container } = render(<SafeHtml html='<form action="/hack"><input type="text"/></form>' />);
	...
	test("multiple on* attributes are all stripped", () => {
		render(
			<SafeHtml
				html='<div onmouseover="evil()" onfocus="hack()" onblur="pwn()">text</div>'
			/>,
		);
