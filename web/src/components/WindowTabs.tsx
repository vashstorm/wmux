...
function getPrimaryAppName(window: WindowSummary, panes: PaneData[] | undefined): string | null {
  const activePane = panes?.find((pane) => pane.active)
  const firstPane = panes?.[0]
  const paneAppCounts = getAppCountsFromPanes(panes)
  const appCounts =
    Object.keys(paneAppCounts).length > 0 ? paneAppCounts : (window.intelligenceAppCounts ?? {})
...
function getWindowDisplayName(window: WindowSummary, panes: PaneData[] | undefined): string {
  const paneAppCounts = getAppCountsFromPanes(panes)
  const appCounts =
    Object.keys(paneAppCounts).length > 0 ? paneAppCounts : (window.intelligenceAppCounts ?? {})
  ...
  const aiApps = rankedApps.map(([app]) => app).filter((app) => app !== "zsh")
...
export function WindowTabs({
  ...
  const tabValue =
    selectedWindowId && windows.some((window) => window.id === selectedWindowId)
      ? selectedWindowId
      : false
    ...
    >
      ...
      >
        {windows.map((window, index) => {
          ...
          const displayName = getWindowDisplayName(window, panes)
                  ...
                  {hasAttentionBadge && (
                    <Box
                      ...
                      sx={{
                        ...
                        lineHeight: 1,
                        color: "#fff",
                        backgroundColor: isAttentionExplicit
                          ? "var(--color-attention-explicit)"
                          : "var(--color-attention)",
