...
const TSX_TARGETS: TypographyTarget[] = [
	...
	},
	{
		...
		migrate: true,
		patterns: [
			/const\s+DETAIL_FONT_SIZE\s*=\s*\{[\s\S]*?"[\d]+px"/gi,
		],
	},
	{
		...
		migrate: true,
		patterns: [
			/const\s+STATS_FONT_SIZE\s*=\s*\{[\s\S]*?"[\d]+px"/gi,
		],
	},
	{
		...
		migrate: true,
		patterns: [
			/fontSize:\s*"[0-9.]+rem"/gi,
			/fontSize:\s*(?:14|10)(?![.\d\w])/gi,
		],
	},
	{
