...
beforeEach(() => {
	...
	vi.mocked(workspaceUrl.fromSelectedPane).mockImplementation((pane: SelectedPane | null) => {
		if (!pane) return null;
		return { connection: pane.targetName, session: pane.session, window: pane.window, pane: pane.pane };
	});
...
describe("useWorkspaceNavigation", () => {
	...
	describe("initial mount with valid URL", () => {
		...
		it("should call listWindows and listPanes when URL has session only, selects first window and active pane", async () => {
			...
			vi.mocked(apiClient.listPanes).mockResolvedValue({
				...
				mode: "local",
				data: [
					{ ID: "p1", Title: "zsh", Index: 0, Active: false, Width: 80, Height: 24, Left: 0, Top: 0 },
					{ ID: "p2", Title: "bash", Index: 1, Active: true, Width: 80, Height: 24, Left: 0, Top: 0 },
				],
			});
		...
		it("should call listWindows, listPanes when URL has window+pane", async () => {
			const location = { connection: "local", session: "session1", window: "window1", pane: "pane1" };
			...
			vi.mocked(apiClient.listPanes).mockResolvedValue({
				...
				window: "window1",
				mode: "local",
				data: [{ ID: "pane1", Title: "test", Index: 0, Active: true, Width: 80, Height: 24, Left: 0, Top: 0 }],
			});
	...
	describe("stale URL handling", () => {
		...
		it("should setSelectedPane(null) when pane not found", async () => {
			const location = { connection: "local", session: "session1", window: "window1", pane: "nonexistent-pane" };
	...
	describe("selectedPane change -> URL write", () => {
		it("should call pushState when selectedPane changes from null to session", () => {
			...
			mockSelectedPane = null;
			vi.mocked(workspaceUrl.getWorkspaceHistoryAction).mockReturnValue("push");
			vi.mocked(workspaceUrl.formatWorkspaceUrl).mockReturnValue("?connection=local&session=session1");
		...
		it("should call replaceState when pane-only change within same window", () => {
			...
			mockSelectedPane = { targetName: "local", session: "session1", window: "window1", pane: "pane1" };
			...
			mockSelectedPane = { targetName: "local", session: "session1", window: "window1", pane: "pane2" };
		...
		it("should call pushState when session changes", () => {
			...
			mockSelectedPane = { targetName: "local", session: "session1" };
			vi.mocked(workspaceUrl.getWorkspaceHistoryAction).mockReturnValue("push");
			vi.mocked(workspaceUrl.formatWorkspaceUrl).mockReturnValue("?connection=local&session=session2");
