...
export function useWorkspaceNavigation(): void {
	...
	useEffect(() => {
		...
		const connectionExists = connections.some(
			(c) => c.targetName === location.connection,
		);
	...
	useEffect(() => {
		...
		if (selectedPane?.targetName === location.connection && selectedPane?.session === location.session) {
			return;
		}
	...
	async function restoreFromLocation(location: ReturnType<typeof parseWorkspaceUrl>): Promise<void> {
		if (!location) return;
		...
		try {
			...
			if (location.window) {
				...
				// Load panes
				const panesResponse = await listPanes(location.connection, location.session, location.window);
