...
describe("AppProvider", () => {
  test("initializes UI settings from the last saved theme", () => {
    localStorage.setItem(
      UI_SETTINGS_STORAGE_KEY,
      JSON.stringify({ theme: "light", windowTheme: "light" }),
    )
    ...
    function ThemeProbe() {
      ...
      return (
        <span data-testid="theme">
          {uiSettings.theme}:{uiSettings.windowTheme}
        </span>
      )
    }
  ...
  function TestComponent() {
    ...
    return (
      <div>
        ...
        <span data-testid="loading-connections">
          {state.loading.connections ? "true" : "false"}
        </span>
        ...
        <button data-testid="set-selected" onClick={() => state.setSelectedTargetName("1")}>
          Select
        </button>
        ...
        <button
          ...
          onClick={() => state.setError({ code: "test", message: "error" })}
        >
          Set Error
        </button>
        ...
        <button
          ...
          onClick={() =>
            state.setSelectedPane({ targetName: "1", session: "s1", window: "w1", pane: "p1" })
          }
        >
          Set Pane
        </button>
        <button
          ...
          onClick={() =>
            state.setConnectionHealth({
              "1": { targetName: "1", status: "online", checkedAt: "2024-01-01T00:00:00Z" },
            })
          }
        >
          Set Health
        ...
        <span data-testid="selected-pane">
          {state.selectedPane
            ? `${state.selectedPane.targetName}:${state.selectedPane.pane}`
            : "null"}
        </span>
        ...
        <span data-testid="panes-loaded-count">
          {(() => {
            const sessionState = state.windows["1:session1"]
            return sessionState?.panesLoaded
              ? String(Object.keys(sessionState.loadedPanes).length)
              : "null"
          })()}
        </span>
...
describe("selection state exclusivity", () => {
  ...
  function TestSelectionComponent() {
    ...
    return (
      <div>
        <button data-testid="set-project" onClick={() => state.setSelectedProject(mockProject)}>
          Set Project
        </button>
        <button
          data-testid="set-pane"
          onClick={() =>
            state.setSelectedPane({ targetName: "conn1", session: "s1", window: "w1", pane: "p1" })
          }
        >
          Set Pane
        </button>
        <button data-testid="clear-project" onClick={() => state.setSelectedProject(null)}>
          Clear Project
        </button>
  ...
  function VoiceTestComponent() {
    ...
    return (
      <div>
        ...
        <span data-testid="voice-confirmation">
          {state.omniPendingConfirmation
            ? `${state.omniPendingConfirmation.confirmationId}:${state.omniPendingConfirmation.skill}`
            : "null"}
        </span>
        <span data-testid="voice-error">{state.omniError || "null"}</span>
        <span data-testid="voice-history-count">{state.omniHistory.length}</span>
        <button data-testid="set-voice-listening" onClick={() => state.setOmniStatus("listening")}>
          Set Listening
        </button>
        <button data-testid="set-voice-error" onClick={() => state.setOmniError("access denied")}>
          Set Error
        </button>
        <button
          data-testid="append-transcript"
          onClick={() => state.appendVoiceTranscript("hello ")}
        >
          Append
        </button>
        <button data-testid="set-transcript" onClick={() => state.setOmniTranscript("done")}>
          Set Transcript
        </button>
        <button
          data-testid="set-confirmation"
          onClick={() => state.setOmniConfirmation({ confirmationId: "c1", skill: "send_to_pane" })}
        >
          Set Confirmation
        </button>
        <button data-testid="clear-confirmation" onClick={() => state.setOmniConfirmation(null)}>
          Clear Confirmation
        </button>
        <button data-testid="clear-voice-error" onClick={() => state.setOmniError(null)}>
          Clear Error
        </button>
        <button
          data-testid="set-voice-history"
          onClick={() =>
            state.setOmniHistory([
              {
                id: "m1",
                conversationId: "c1",
                role: "user",
        ...
        <button data-testid="clear-voice-history" onClick={() => state.setOmniHistory([])}>
          Clear History
        </button>
      </div>
    )
